...
const USER_IDENTITY = `user-text-${Math.round(Math.random() * 1000)}`;
...
class ConnectionManager {
  ...
  constructor() {
    ...
    this.room.on(RoomEvent.ConnectionStateChanged, (state: ConnectionState) => {
      if (state === ConnectionState.Connected) this.onStatusChange('connected');
      else if (state === ConnectionState.Connecting || state === ConnectionState.Reconnecting) this.onStatusChange('connecting');
      else if (state === ConnectionState.Disconnected) this.onStatusChange('disconnected');
    });
  ...
  public async connect() {
    ...
    try {
      ...
      await this.room.connect(LIVEKIT_CONFIG.SERVER_URL, token);
    } catch (error) {
  ...
  private async fetchToken(): Promise<string> {
    const response = await fetch(LIVEKIT_CONFIG.TOKEN_ENDPOINT, {
      ...
      body: JSON.stringify({ room_name: AI_CHAT_ROOM_NAME, identity: USER_IDENTITY }),
    });
  ...
  public sendMessage(message: string): boolean {
    if (this.room.state === ConnectionState.Connected) {
      this.room.localParticipant.publishData(new TextEncoder().encode(message), { reliable: true });
      return true;
    }
    return false;
  }
